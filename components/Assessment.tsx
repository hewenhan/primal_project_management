...
export const Assessment: React.FC<Props> = ({ language, onComplete, onImport }) => {
  ...
  if (!ASSESSMENT_QUESTIONS || ASSESSMENT_QUESTIONS.length === 0) {
  ...
  // Safeguard if index is out of bounds (should not happen with logic below, but safe for render)
  if (!currentQ) {
  ...
  }, [answers, currentStep, loading, currentQ]);
  ...
  return (
    <div className="max-w-3xl mx-auto p-8 bg-surface border border-gray-800 rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] flex flex-col min-h-[600px] relative">
                ...
                return (
                    <button
                        ...
                        className={`group relative p-6 rounded-xl border text-left transition-all duration-200 flex items-center justify-between ${
                            ...
                            : 'bg-black/40 border-gray-700 text-gray-400 hover:border-gray-500 hover:bg-gray-900'
                        }`}
                    >
